...
async function makeToken(env) {
  const payload = b64url(new TextEncoder().encode(JSON.stringify({ admin: true, exp: Date.now() + 8 * 60 * 60 * 1000 })));
...
async function verifyToken(env, token) {
  ...
  try {
    ...
    if (!data.admin || data.exp < Date.now()) return null;
    return data;
  } catch { return null; }
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/auth/login" && request.method === "POST") {
      ...
      if (!env.ADMIN_PASSWORD || !env.SESSION_SECRET) return json({ error: "AUTH_NOT_CONFIGURED" }, 500, request, env);
      if (body.password !== env.ADMIN_PASSWORD) return json({ error: "INVALID_CREDENTIALS" }, 401, request, env);
      return json({ ok: true, token: await makeToken(env) }, 200, request, env);
    }

    if (url.pathname === "/api/auth/me" && request.method === "GET") {
      const user = await requireAuth(request, env);
      return json({ authenticated: !!user, admin: !!user }, user ? 200 : 401, request, env);
    }
